...
-- This doesn't actually return the rows in order, only in partial order. That's
-- because the LockRows node might need to fetch a new different value in
-- EvalPlanQual. PostgreSQL does the same, but it's worse in GPDB: we cannot
...
-- GPDB_96_MERGE_FIXME: Before 9.6, we used to create the Gather Motion plan
-- anyway. That seems a bit bogus, but maybe we should keep doing that?
1: begin;
1: explain select * from t_lockmods order by c for update;
